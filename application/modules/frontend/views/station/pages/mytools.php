		...
		<div class="tab-content main-tab-content">
			<div class="panel panel-default tab-pane fade in active" id="maintoolscreen" style="height:705px;margin-bottom:0px;">
				<div class="panel-heading custom-panel-heading mn-heading" style="text-align:left;padding-top: 5px !important;border-bottom: 0px;background-color:#fff !important;">
					...
					<span class="pull-right-close" style="padding-top: 0px;padding-right: 0px;margin-top: -18px;"><a href="javascript:ShowObjectWithEffect('Layer528', 0, 'dropup', 500, 'easeInBounce');ShowObjectWithEffect('Layer1', 1, 'dropdown', 500, 'easeInBounce');" class="btn-custom-close" style="padding:6px 12px;font-size:22px;">x</a></span>
				</div>
				<div class="panel-inner">
					<div id="myperformance" class="mytool-div">
						...
						<div id="wb_Text1468" style="position:absolute;left:124px;top:239px;width:60px;height:16px;text-align:center;z-index:3370;">
							<span style="color:#3C3C3C;font-family:Arial;font-size:12px;"><strong><?php echo $product_history['viewsHomePg'] ? $product_history['viewsHomePg']:0;?></strong></span>
						</div>
						...
						<div id="wb_Text1472" style="position:absolute;left:124px;top:282px;width:60px;height:16px;text-align:center;z-index:3371;">
						...
						<span style="color:#2688E4;font-family:Arial;font-size:12px;"><strong>Likes</strong></span></div>
						<div id="wb_Text1471" style="position:absolute;left:69px;top:282px;width:51px;height:16px;z-index:3379;text-align:left;">
						<span style="color:#2688E4;font-family:Arial;font-size:12px;"><strong>Shares</strong></span></div>
						...
						<?php foreach ($aproduct_stats as $key=>$active_data) { ?>
							<?php if($key <= 5) { ?>
							<div id="wb_Shape15" style="padding:5px;float:left;width:55px;height:55px;margin-bottom:5px;">
								<?php $image=$active_data['images'];
								 if(!empty($image)) { ?>
								<img src="<?php echo asset_url();?><?php echo $image;?>" id="Shape15" alt="" style="width:50px;border-radius:4px;">
								<?php } ?>
							</div>
								...
								<div class="row">
									...
									<div class="col-sm-4 text-left">
										...
										<div style="margin-left:25px;">
											...
											<span style="color:#2D2D2D;font-family:Arial;font-size:13px;">Free Space <?php echo ($ois_plan - $inquiry_storage)?>MB</span>
										</div>
									</div>
								</div>
							</div>
			...
			<div class="panel panel-default tab-pane fade in active" id="statstoolscreen" style="display:none;height:705px;margin-bottom:0px;">
				<div class="panel-heading custom-panel-heading sts-heading" style="text-align:left;padding-top: 5px !important;border-bottom: 0px;background-color:#fff !important;">
					...
					<span class="pull-right-close" style="padding-top: 0px;padding-right: 0px;margin-top: -18px;"><a href="javascript:closeStatsDiv();" class="btn-custom-close" style="padding:6px 12px;font-size:22px;">x</a></span>
				</div>
				<div class="panel-inner">
					<div id="myperformance_stats" class="mytoolsts-div" style="max-height: 615px;">
						<div class="row" style="margin:0px;">
						...
						<div class="row" style="margin:0px;">
							...
							<div class="col-sm-9">
								...
								<br><br>
								<div >
								<div class="row" id="busi_stats_area" ><!-- style="overflow: scroll;height: 480px;" -->
									<div class="col-sm-12">
										<table class="skyblue">
											<thead>
												<tr>
								...
								<br>
								<div class="row">
									<div class="pull-right page_display" style="padding-right:20px;">
										<strong>Page&nbsp;&nbsp;&nbsp; </strong>
										<?php for ($i = 1; $i <= $busi_history_pages['pages']; $i++) { ?>
										<a id="pagination_<?php echo $i;?>" href="javascript:showNextBusinessStats(<?php echo $i;?>);" class="pagination_tool<?php if($i == 1) { ?> page-active<?php } ?>"><?php echo $i;?></a>
										<?php } ?>
									</div>
								</div>
								</div>
					...
					<div id="myaproduct_stats" style="display:none;overflow-y:scroll;" class="mytoolsts-div">
						...
						<div class="row" style="margin:0px;">
							...
							<div class="col-sm-9">
								...
								<br>
								<div class="row">
									<div class="pull-right page_displayaproduct" style="padding-right:20px;">
										<strong>Page&nbsp;&nbsp;&nbsp; </strong>
										<?php for ($i = 1; $i <= $aproduct_history_pages['pages']; $i++) { ?>
										<a id= "aproductpagination_<?php echo $i;?>" href="javascript:showNextAProductStats(<?php echo $i;?>);" class="pagination_tool<?php if($i == 1) { ?> page-active<?php } ?>"><?php echo $i;?></a>
										<?php } ?>
									</div>
								</div>
								</div>
...
function searchAproductStats() {
	$.post(base_url+"mystation/aproduct/stats",{country_name: $("#apcountry_id").val(), product_name: $("#apparo_name").val(), page: 1},function(data) {
		...
		$(".page_displayaproduct a").removeClass('page-active');
		$("#aproductpagination_"+page).addClass('page-active');
	},'html');
}
...
function showNextBusinessStats(page) {
	$.post(base_url+"mystation/business/stats",{country_name: $("#bcountry_id").val(), from_date: $("#bfrom_date").val(), to_date: $("#bto_date").val(), page: page},function(data) {
		$("#busi_stats_area").html(data);
		$(".page_display a").removeClass('page-active');
		$("#pagination_"+page).addClass('page-active');
	},'html');	
}
...
function showNextProductStats(page) {
	$.post(base_url+"mystation/product/stats",{country_name: $("#pcountry_id").val(), from_date: $("#pfrom_date").val(), to_date: $("#pto_date").val(), product_name: $("#pro_name").val(), page: page},function(data) {
		$("#product_stats_area").html(data);
		$(".page_displayproduct a").removeClass('page-active');
		$("#productpagination_"+page).addClass('page-active');
	},'html');
}
