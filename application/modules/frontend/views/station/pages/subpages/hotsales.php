...
<div class="panel-heading custom-panel-heading" style="text-align:center;padding-top: 5px !important;">
	...
	<?php if(count($hotsales) > 0) { ?>
	<a href="javascript:deleteHoteSaleProduct();" class="btn-custom-product" id="product_del_btn">
...
<div class="panel-body panel-body-custom" id="company_product_info">
	<div id="product_list">
		<div class="product-list-outer">
			<div class="row" id="title-menu">
				<div class="col-sm-9">
					<div class="product-list-title">
						Manage Your Hot Sale Menu
					</div>
					<br>
					<div class="product-list-text">
						Select the required products, then click on add icon to add to your Hot Sale menu..
						To display your products click on main & Sub paroducts drop down list, make a selection and click <br>on search icon. You can also type a specific product name in search field then click on search icon..
						<br><br>
						<span class="help-text-red text-xs">To add a product to your Hot Sale menu, the product must be added first to your Main or Sub Products.</span>
					</div>
					<br>
						  	...
						  	<ul class="dropdown-menu" style="margin-left:15px;">
						    	...
						    	<li class="dropdown">
						    		<?php if(count($mpro['subproducts']) > 0) { ?>
						      		...
						      		<?php } else { ?>
						      		<a href="javascript:selectMSearchProduct(0,'<?php echo $mpro['id'];?>',``,`<?php echo $mpro['name'];?>`);" class="mp-option-row"><?php echo $mpro['name'];?></a>
						      		<?php } ?>
						      		<?php if(count($mpro['subproducts']) > 0) { ?>
						      		<ul class="dropdown-menu">
						      			<?php foreach ($mpro['subproducts'] as $spro) { ?>
						        		<li><a href="javascript:selectMSearchProduct(<?php echo $spro['id'];?>,'<?php echo $mpro['id'];?>',`<?php echo $spro['name'];?>`,`<?php echo $mpro['name'];?>`);"><?php echo $spro['name'];?></a></li>
						        		<?php } ?>
						       		</ul>
						       		<?php } ?>
						    	</li>
			...
			<div id="search_result">
				<?php foreach ($hotsales as $product) { ?>
				<div class="row" style="margin:0px;">
					<div class="col-sm-12 product-list-row">
						<div class="col-sm-1 product-select">
							<input type="checkbox" name="del-hot-pro-id" id="del-hot-pro-id" value="<?php echo $product['id'];?>">
						</div>
						<div class="col-sm-2">
...
<script>
function selectMSearchProduct(scat_id,mcat_id,name,mname) {
...
function addHoteSaleProduct() {
	...
	if(checkcount <= 0 ){
		...
		$.post(base_url+"mystation/desksite/addhotsale",{item_id: item_id},function(resp){
			if(resp.status == 0) {
				alert(resp.msg);
			} else {
				//alert(resp.msg);
...
function deleteHoteSaleProduct() {
	var checkcount = 0;
	var item_id = "";
	$('input[name="del-hot-pro-id"]:checked').each(function() {
		if(item_id != "") {
			item_id = item_id+","+this.value;
		} else {
			item_id = this.value;
	...
	if(checkcount <= 0 ){
		...
		$.post(base_url+"mystation/desksite/deletehotsale",{item_id: item_id},function(resp){
			if(resp.status == 0) {
				alert(resp.msg);
			} else {
				//alert(resp.msg);
